...
import { downloadBookingVoucher } from '@/lib/voucher-pdf'
...
function Inner() {
  ...
  useEffect(() => {
    if (ref && type !== 'custom') {
      fetch(`/api/bookings/lookup/${ref}`).then(r => r.json()).then(setData).catch(() => {})
    }
      ...
      <section className="pt-32 pb-24">
        <div className="container max-w-2xl text-center">
          ...
          <h1 className="font-display text-4xl md:text-5xl font-semibold mb-4">
            {isCustom ? 'Request received!' : 'Booking confirmed!'}
          </h1>
          <p className="text-lg text-muted-foreground mb-6">
            Kia ora! We&apos;ve received your {isCustom ? 'custom journey request' : 'tour booking'} and our team will be in touch within 24 hours.
          </p>
          {ref && (
            <div className="inline-block bg-white border-2 border-dashed border-accent rounded-2xl px-8 py-5 mb-8">
          ...
          {data?.booking && (
            <div className="mb-8">
              <Button size="lg"
                onClick={() => downloadBookingVoucher(data.booking, data.tour, settings)}
                className="bg-primary hover:bg-primary/90 text-primary-foreground rounded-full h-12 px-8 font-semibold">
                <Download className="mr-2 h-4 w-4" /> Download Voucher (PDF)
              </Button>
              <p className="text-xs text-muted-foreground mt-2">Present this branded voucher on the day of your tour.</p>
            </div>
          )}
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            <Button asChild className="bg-accent hover:bg-accent/90 text-accent-foreground rounded-full h-12 px-8"><Link href="/tours">Explore More Tours <ArrowRight className="ml-2 h-4 w-4" /></Link></Button>
